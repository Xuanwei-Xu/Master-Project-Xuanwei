...
def main(args):
    ...
    net = resnet50()  
    model_weight_path = args.weights  # The path of pre-trained weights
    assert os.path.exists(model_weight_path), "file {} does not exist.".format(model_weight_path)
    net.load_state_dict(torch.load(model_weight_path, map_location=device), strict=False)  # Load pre-trained weights in model

    in_channel = net.fc.in_features
    net.fc = nn.Linear(in_channel, args.num_classes)
    net.to(device) 

    # Create optimiser (stochastic gradient descent), set learning rate, momentum, weight decay factor
    optimizer = optim.SGD(net.parameters(), lr=args.lr, momentum=0.9, weight_decay=1E-4, nesterov=True)

    # Create a cosine function for updating the learning rate, which is 0.1 times what it was at the start when training is complete
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_classes', type=int, default=4)  # Number of classes
    parser.add_argument('--epochs', type=int, default=100)  # Training epoch
    parser.add_argument('--batch-size', type=int, default=16) 
    parser.add_argument('--lr', type=float, default=0.001)  # initial learning rate
    parser.add_argument('--lrf', type=float, default=0.1) 
    parser.add_argument('--data-path', type=str,
                        default="../brain_dataset_24b")  
    parser.add_argument('--weights', type=str, default='./resnet50-pre.pth',  # The path of pre-trained model
