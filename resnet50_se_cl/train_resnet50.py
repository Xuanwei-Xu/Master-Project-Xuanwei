...
def main(args):
    ...
    model_weight_path = args.weights  # 预训练权重路径
    assert os.path.exists(model_weight_path), "file {} does not exist.".format(model_weight_path)
    model_weight = torch.load(model_weight_path, map_location=device)["model"]
    model_weight = dict((keys.replace("module.encoder.", ""), val) for keys, val in model_weight.items())
    print(model_weight.keys())
    print(net.load_state_dict(model_weight, strict=False))  # 加载预训练模型至模型
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', type=str,
                        default='../se-pycontrast/save/MoCov2_resnet50_RGB_Jig_False_moco_aug_B_mlp_0.2_cosine/last_model.pth',
                        # 预训练模型路径
